...
import { ChevronLeft, Code, Cpu, Eye, Globe, Save, Settings, ShieldAlert } from 'lucide-react';
...
export default function CategoryForm({ initialData, isEditing = false }) {
  ...
  useEffect(() => {
    if (!isLoading && !hasPermission('categories', 'edit')) {
  ...
  useEffect(() => {
    fetch('/api/admin/categories')
      ...
      .then((data) => {
        if (data.success) {
          setCategories(data.data.filter((category) => category._id !== initialData?._id));
        }
      })
      .catch(() => {});
  }, [initialData?._id]);
    ...
    <form onSubmit={handleSubmit} className="space-y-8 max-w-6xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-30 bg-slate-50/90 backdrop-blur-md py-4 -mx-4 px-4 border-b border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">{isEditing ? 'Edit Category' : 'Create New Category'}</h1>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-6">
                <button type="button" onClick={() => setActiveTab('editor')} className={`text-sm font-bold uppercase tracking-wider pb-1 border-b-2 transition-all ${activeTab === 'editor' ? 'border-violet-600 text-violet-600' : 'border-transparent text-slate-400'}`}>Category Content</button>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <select value={formData.parent || ''} onChange={(e) => setFormData({ ...formData, parent: e.target.value })} className={inputClass}>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
            ...
            <select value={formData.status} onChange={(e) => setFormData({ ...formData, status: e.target.value })} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-violet-500 outline-none transition-all font-bold text-slate-700 text-xs uppercase">
              <option value="active">Published Category</option>
