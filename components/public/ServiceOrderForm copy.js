...
.f-pill.sent { background: var(--warn-bg); color: var(--warn); border: 1px solid rgba(198,154,0,0.2); }
.f-pill.ok   { background: var(--success-bg); color: var(--success); border: 1px solid rgba(58,158,106,0.2); }

.f-otp-wrap { display: flex; gap: 6px; margin-top: 8px; animation: f-in 0.2s ease; }
.f-otp-input {
  flex: 1; background: #fafaf8; border: 1px solid var(--border);
...
.f-phone-row { display: flex; gap: 6px; }
.f-country-select {
  background: #fafaf8; border: 1px solid var(--border);
  border-radius: var(--radius-sm); padding: 9px 8px;
   font-size: 12px; color: var(--text);
  outline: none; cursor: pointer; width: 100px; flex-shrink: 0;
...
.f-phone-prefix { font-size: 12px; color: var(--text-muted); white-space: nowrap; }
.f-phone-inner input {
...
.f-addr-list {
  ...
  background: var(--white); border: 1px solid var(--border-focus);
...
export default function ServiceOrderForm({ serviceId, uploadLabel, defaultPhoneCountry = 'US' }) {
  const addressInputRef = useRef(null);
  const fileInputRef = useRef(null);
  const initialPhoneCountry = normalizePhoneCountry(defaultPhoneCountry);

  const [formData, setFormData] = useState({ fullName: '', email: '', phone: '', phoneCountry: initialPhoneCountry, address: '' });
  ...
  useEffect(() => {
    try {
      ...
      setTimeout(() => setFormData((p) => p.address ? p : { ...p, address: data.location }), 0);
    } catch {}
  }, []);
  ...
  const setField = (field, value) => {
    ...
    if (field === 'email') { setEmailVerified(false); setOtpSent(false); setOtp(''); setEmailVerificationToken(''); }
  };
  ...
  const selectedPhoneCountry = phoneCountries.find((c) => c.code === formData.phoneCountry) || phoneCountries.find((c) => c.code === initialPhoneCountry);
  ...
  const saveLocation = (loc) => { try { localStorage.setItem('userLocation', JSON.stringify(loc)); } catch {} };
  ...
  const selectSuggestion = async (s) => {
    ...
    new google.maps.Geocoder().geocode({ placeId: s.place_id }, (results, status) => {
      if (status === 'OK' && results?.[0]) {
        ...
        const { city, state } = parseAddressComponents(results[0].address_components);
        const address = results[0].formatted_address || fallback;
        setFormData((p) => ({ ...p, address }));
        saveLocation({ location: address, lat: loc.lat(), lng: loc.lng(), city, state });
      } else if (fallback) setFormData((p) => ({ ...p, address: fallback }));
  ...
  const addFiles = (fileList) => {
    ...
    setDocuments((p) => [...p, ...files.map((f) => ({ file: f, url: URL.createObjectURL(f) }))]);
  ...
  const submitOrder = async (e) => {
    ...
    try {
      ...
      payload.append('phone', ph.formatted);
      payload.append('phoneCountry', ph.country);
      payload.append('address', formData.address);
      payload.append('emailVerificationToken', emailVerificationToken);
      documents.forEach((d) => payload.append('documents', d.file));
      const res = await fetch('/api/orders', { method: 'POST', body: payload });
      const data = await res.json();
      if (data.success) {
        toast.success('Order submitted!');
        setFormData({ fullName: '', email: '', phone: '', phoneCountry: initialPhoneCountry, address: '' });
        documents.forEach((d) => URL.revokeObjectURL(d.url));
        setDocuments([]); if (fileInputRef.current) fileInputRef.current.value = '';
        setEmailVerified(false); setOtpSent(false); setOtp(''); setEmailVerificationToken('');
      } else toast.error(data.message || 'Failed to submit');
    } catch { toast.error('Failed to submit order'); }
    finally { setSubmitting(false); }
  };

  const step1Done = !!formData.fullName && emailVerified;
   ...
   <div className="grid grid-cols-2 gap-4">
              ...
              <div className="f-field">
                ...
                <div className="f-email-row">
                  ...
                  >
                    {emailVerified ? '✓ Done' : verifying ? '…' : otpSent ? 'Resend' : 'Verify'}
                  </button>
                </div>
                {otpSent && !emailVerified && <span className="f-pill sent">📨 Check your inbox for OTP</span>}
            ...
            <div className="f-card">
              <div className="f-card-head">
                <div className="f-card-icon">📍</div>
                <p className="f-card-label">Contact & Location</p>
              </div>
        ...
        <div className="f-modal-bg" role="dialog" aria-modal="true">
          ...
          <div className="f-modal">
            <div className="f-modal-head">
              <div>
                <h3>{activePreview.file.name}</h3>
                <p>{formatFileSize(activePreview.file.size)}</p>
              </div>
              <button type="button" className="f-modal-close" onClick={() => setActivePreview(null)}>✕</button>
            </div>
